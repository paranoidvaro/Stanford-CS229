...
def initialize_mdp_data(num_states):
    ...
    probability_tensor = np.ones((num_states, num_states, 2)) / num_states
    ...
    return {
        ...
        'transition_probs': probability_tensor,
...
def choose_action(state, mdp_data):
    ...
    # *** START CODE HERE ***
    action_space = [0, 1]
    probability_tensor = mdp_data['transition_probs']
    values = mdp_data['value']

    value_a = np.dot(probability_tensor[state, :, action_space[0]], values)
    value_b = np.dot(probability_tensor[state, :, action_space[1]], values)
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    for action in action_space:
        ...
        for state in range(n_states):
            total_num_transitions = np.sum(transition_counts[state, :, action])

            if total_num_transitions > 0:
                probability_tensor[state, :, action] = (
                    transition_counts[state, :, action] / total_num_transitions
                )

    for state in range(n_states):
        if reward_counts[state, 1] > 0:
            reward[state] = reward_counts[state, 0] / reward_counts[state, 1]
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    n_states = mdp_data['num_states']
    probability_tensor = mdp_data['transition_probs']
    n_actions = probability_tensor.shape[2]

    iteration = 0
    while True:
        new_value = np.array([np.max([probability_tensor[s, :, a].dot(mdp_data['value']) for a in range(n_actions)]) for s in range(n_states)])
        ...
        iteration += 1
        if max_diff < tolerance or iteration > 1:
            break
