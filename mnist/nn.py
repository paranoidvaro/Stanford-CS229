...
def softmax(x):
    ...
    """
    # *** START CODE HERE ***
    exp_norm = np.exp(x - np.max(x, axis=1, keepdims=True)) 
    
    return exp_norm / np.sum(exp_norm, axis=1, keepdims=True)
    # *** END CODE HERE ***
...
def forward_prop(data, labels, params):
    ...
    loss = -np.mean(np.sum(labels * np.log(y_hat), axis=1))
...
def backward_prop(data, labels, params, forward_prop_func):
    ...
    grad_w2 = 1 / rows * np.dot(a1.T, grad_z2)

    grad_b2 = 1 / rows * np.sum(grad_z2, axis=0, keepdims=True)

    grad_z1 = np.dot(grad_z2, params['W2'].T) * (a1 * (1 - a1))

    grad_w1 = 1 / rows * np.dot(data.T, grad_z1)

    grad_b1 = 1 / rows * np.sum(grad_z1, axis=0, keepdims=True)
...
def backward_prop_regularized(data, labels, params, forward_prop_func, reg):
    ...
    grad_w2 = 1 / rows * np.dot(a1.T, grad_z2) + reg * params['W2'] * 2

    grad_b2 = 1 / rows * np.sum(grad_z2, axis=0, keepdims=True)

    grad_z1 = np.dot(grad_z2, params['W2'].T) * (a1 * (1 - a1))

    grad_w1 = 1 / rows * np.dot(data.T, grad_z1) + reg * params['W1'] * 2
...
def run_train_test(name, all_data, all_labels, backward_prop_func, num_epochs, plot=True):
    ...
    np.save(f'{name}_params.npy', params)
