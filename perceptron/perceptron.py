...
def predict(state, kernel, x_i):
    ...
    for weight, feature_vector in state:

        kernel_value = kernel(feature_vector, x_i)

        weighted_kernel = weight * kernel_value

        value += weighted_kernel
...
def update_state(state, kernel, learning_rate, x_i, y_i):
    ...
    if error != 0:
        ...
        new_state = (learning_rate * error, x_i)
...
def polynomial_kernel(a, b, degree=2, r=1):
    ...
    Args:
        ...
        r: The independent term in the polynomial
    """
    # Compute the dot product of the two vectors
    dot_product = np.dot(a, b)

    # Compute the Polynomial kernel function of the dot product
    kernel_value = (dot_product + r) ** degree
...
def main():
    ...
    train_perceptron('polynomial', polynomial_kernel, 0.5)
